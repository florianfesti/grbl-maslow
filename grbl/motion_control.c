...
//Segment straight lines to ensure linear movement with non cartesian systems
#ifdef SEGMENTED_LINES
void mc_segmented_line(float *position, float *target, plan_line_data_t *pl_data)
{
  float mm_per_line_segment = 2.0;  //TODO: move to settings
  float mm_of_travel = hypot(target[X_AXIS] - position[X_AXIS],
		  target[Y_AXIS] - position[Y_AXIS]);
  uint16_t segments = floor(mm_of_travel / mm_per_line_segment) + 1;
  float intermediate_target[N_AXIS];
  ...
  for (uint16_t i = 1; i<=segments; i++) {
	  ...
	  intermediate_target[a] = ((target[a] * i) +
				    position[a] * (segments-i)) / segments;
      }
      mc_line(intermediate_target, pl_data);
      // Bail on system abort. Runtime command check already performed by mc_line.
      if (sys.abort) { return; }
  }
}
#endif
